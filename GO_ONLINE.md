...
| **แพตช์ `supabase/debt.sql`** (หนี้สินและลูกหนี้: ตาราง `debts`, `debt_entries`, ทับ `clear_shop_data`) | ⚠️ ต้องรันหลัง `card.sql` — ไม่รัน = หัวข้อหนี้สินว่างเปล่า (ส่วนอื่นยังใช้ได้) |
...
ลำดับการรัน SQL บนฐานข้อมูลที่ติดตั้งไปแล้ว: `fix.sql` → `card.sql` → `debt.sql` (รันซ้ำได้ ไม่ลบข้อมูล)
...
`card.sql` (บัตรเครดิตทั้งระบบ), `debt.sql` (หนี้สินและลูกหนี้),
